...
$source = 'chapters\chapter-12\src\Chapter12Checks.dpr'
foreach ($architecture in @('Win32', 'Win64')) {
  $compiler = if ($architecture -eq 'Win32') { 'dcc32' } else { 'dcc64' }
  $output = ".deps\build\$architecture\chapter-12"
...
foreach ($architecture in @('Win32', 'Win64')) {
  ...
  $env:CH12_SQLITE_DATABASE = $sqliteDatabase.FullName
  $env:FIRESTORE_FBCLIENT =
    (Resolve-Path ".deps\firebird\$architecture\fbclient.dll").Path
  $executable = ".deps\build\$architecture\chapter-12\Chapter12Checks.exe"
...
Write-Output 'Capítulo 12 aprovado em SQLite/Firebird e Win32/Win64.'
