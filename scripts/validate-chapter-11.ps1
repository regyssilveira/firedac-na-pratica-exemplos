...
$source = 'chapters\chapter-11\src\Chapter11Checks.dpr'
foreach ($architecture in @('Win32', 'Win64')) {
  $compiler = if ($architecture -eq 'Win32') { 'dcc32' } else { 'dcc64' }
  $output = ".deps\build\$architecture\chapter-11"
...
foreach ($architecture in @('Win32', 'Win64')) {
  ...
  $env:CH11_SQLITE_DATABASE = $sqliteDatabase.FullName
  $env:FIRESTORE_FBCLIENT =
    (Resolve-Path ".deps\firebird\$architecture\fbclient.dll").Path
  $executable = ".deps\build\$architecture\chapter-11\Chapter11Checks.exe"

  foreach ($driver in @('SQLite', 'FB')) {
    $env:CH11_DRIVER = $driver
...
Write-Output 'Capítulo 11 aprovado em SQLite/Firebird e Win32/Win64.'
