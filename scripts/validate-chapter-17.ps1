...
$source = 'chapters\chapter-17\src\Chapter17Checks.dpr'
foreach ($architecture in @('Win32', 'Win64')) {
  $compiler = if ($architecture -eq 'Win32') { 'dcc32' } else { 'dcc64' }
  $output = ".deps\build\$architecture\chapter-17"
...
$env:CH17_CSV_FILE = (Resolve-Path 'chapters\chapter-17\data\targets.csv').Path
foreach ($architecture in @('Win32', 'Win64')) {
  $env:CH17_SQLITE_DATABASE = (Get-ChildItem ".deps\firestore\m0-*-$architecture.sqlite" |
...
Write-Output 'Capítulo 17 aprovado em SQLite/Firebird e Win32/Win64.'
