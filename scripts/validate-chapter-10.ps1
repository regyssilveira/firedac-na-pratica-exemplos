...
$source = 'chapters\chapter-10\src\Chapter10Checks.dpr'
foreach ($architecture in @('Win32', 'Win64')) {
  $compiler = if ($architecture -eq 'Win32') { 'dcc32' } else { 'dcc64' }
  $output = ".deps\build\$architecture\chapter-10"
...
foreach ($architecture in @('Win32', 'Win64')) {
  ...
  $env:CH10_SQLITE_DATABASE = $sqliteDatabase.FullName
  $env:FIRESTORE_FBCLIENT =
    (Resolve-Path ".deps\firebird\$architecture\fbclient.dll").Path
  $executable = ".deps\build\$architecture\chapter-10\Chapter10Checks.exe"

  foreach ($driver in @('SQLite', 'FB')) {
    $env:CH10_DRIVER = $driver
    foreach ($mode in @('commit', 'rollback', 'nested', 'isolation', 'helper', 'retry')) {
      & $executable $mode
...
Write-Output 'Capítulo 10 aprovado em SQLite/Firebird e Win32/Win64.'
