...
$source = 'chapters\chapter-01\src'

foreach ($architecture in @('Win32', 'Win64')) {
  $compiler = if ($architecture -eq 'Win32') { 'dcc32' } else { 'dcc64' }
  $output = ".deps\build\$architecture\chapter-01"
  New-Item -ItemType Directory -Force -Path $output | Out-Null
  foreach ($project in @('Chapter01Checks.dpr', 'Chapter01Vcl.dpr')) {
...
$database = Get-ChildItem '.deps\firestore\m0-*.fdb' |
  Sort-Object LastWriteTime -Descending | Select-Object -First 1
if (-not $database) { throw 'Banco Firebird descartável não encontrado.' }

$env:FIRESTORE_DB_HOST = '127.0.0.1'
$env:FIRESTORE_DB_PORT = '3050'
$env:FIRESTORE_DB_NAME = $database.FullName
$env:FIRESTORE_DB_USER = 'FIRESTORE_APP'
$env:FIRESTORE_DB_PASSWORD = $AppPassword

foreach ($architecture in @('Win32', 'Win64')) {
  $output = ".deps\build\$architecture\chapter-01"
  $checks = Join-Path $output 'Chapter01Checks.exe'
  $vcl = Join-Path $output 'Chapter01Vcl.exe'
  $env:FIRESTORE_FBCLIENT = (Resolve-Path ".deps\firebird\$architecture\fbclient.dll").Path
  $env:CH01_SQLITE_DATABASE = Join-Path (Resolve-Path '.deps\firestore').Path `
    "chapter01-$architecture.sqlite"

  foreach ($mode in @('sqlite', 'parameter', 'firebird', 'missing-client')) {
    & $checks $mode
    if ($LASTEXITCODE -ne 0) { throw "Teste $mode para $architecture falhou." }
  }
  ...
  foreach ($driver in @('SQLite', 'FB')) {
    ...
    $resultFile = Join-Path (Resolve-Path '.deps').Path `
      "chapter01-vcl-$architecture-$driver.txt"
    $env:CH01_AUTORUN_RESULT = $resultFile
    ...
    if ($process.ExitCode -ne 0) {
      ...
      throw "Autoteste VCL $driver para $architecture falhou: $diagnostic"
    }
    if ((Get-Content $resultFile -Raw) -ne 'OK') {
      throw "Autoteste VCL $driver para $architecture não confirmou o resultado."
    }
  }
  Remove-Item Env:CH01_AUTORUN -ErrorAction SilentlyContinue
  Remove-Item Env:CH01_AUTORUN_RESULT -ErrorAction SilentlyContinue
}

$dfm = Get-Content "$source\Chapter01.MainForm.dfm" -Raw
...
Write-Output 'Capítulo 1 aprovado: cinco exemplos, Win32 e Win64, SQLite e Firebird.'
